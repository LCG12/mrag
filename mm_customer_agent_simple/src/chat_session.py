...
class ChatAgentService:
    ...
    def answer_chat_turn(
        ...
        has_current_images = bool(image_paths or image_base64)
        with self._answer_lock:
            rewritten_query = (
                message
                if has_current_images
                else self.agent.generator.rewrite_current_turn(rewrite_history, message)
            )
            result = self.agent.answer(
                ...
                rewrite_history=rewrite_history if has_current_images else None,
                turn_mode=turn_mode,
            )
