...
class HybridRetriever:
    def __init__(self, settings: Settings):
        ...
        self.embedder = EmbeddingClient(settings) if settings.enable_vector_retrieval else None
        self.qdrant = QdrantStore(settings) if settings.enable_vector_retrieval else None
        self.bm25 = BM25Store.load(Path(settings.storage_dir) / "bm25.pkl")

    def retrieve(self, query: str) -> List[RetrievedChunk]:
        vector_hits = []
        if self.embedder is not None and self.qdrant is not None:
            query_vector = self.embedder.embed_query(query)
            vector_hits = self.qdrant.search(
                query_vector, top_k=self.settings.vector_top_k
            )
