...
class Settings:
    ...
    enable_vector_retrieval: bool = _get_bool(
        "ENABLE_VECTOR_RETRIEVAL", bool(os.getenv("DASHSCOPE_API_KEY", ""))
    )
    enable_rerank: bool = _get_bool(
        "ENABLE_RERANK", bool(os.getenv("DASHSCOPE_API_KEY", ""))
    )
    ...
    api_vlm_api_key: str = os.getenv(
        "API_VLM_API_KEY", os.getenv("DASHSCOPE_API_KEY", "")
    )
...
def get_settings() -> Settings:
    ...
    if not settings.llm_api_key:
        raise RuntimeError(
            "请先设置 LLM_API_KEY（或兼容配置 DASHSCOPE_API_KEY）。"
        )
    if settings.enable_vector_retrieval and not settings.embedding_api_key:
        raise RuntimeError(
            "ENABLE_VECTOR_RETRIEVAL=true 时必须设置 EMBEDDING_API_KEY。"
        )
    if settings.enable_rerank and not settings.rerank_api_key:
        raise RuntimeError("ENABLE_RERANK=true 时必须设置 RERANK_API_KEY。")
    return settings
